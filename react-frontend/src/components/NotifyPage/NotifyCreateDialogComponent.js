...
const NotifyCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.organizer)) {
                error["organizer"] = `Organizer field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            venue: _entity?.venue,tarikh: _entity?.tarikh,organizer: _entity?.organizer,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="organizer" className="w-full mb-3 p-inputtext-sm" value={_entity?.organizer} onChange={(e) => setValByKey("organizer", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["organizer"]) ? (
              <p className="m-0" key="error-organizer">
                {error["organizer"]}
              </p>
            ) : null}
          </small>
