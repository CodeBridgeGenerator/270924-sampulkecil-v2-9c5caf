...
const LatihanCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.NoRujukan)) {
                error["NoRujukan"] = `NoRujukan field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            NoRujukan: _entity?.NoRujukan,kategori: _entity?.kategori,status: _entity?.status,tajuk: _entity?.tajuk,
    ...
    return (
        <Dialog header="Create Latihan" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                <label htmlFor="NoRujukan">NoRujukan:</label>
                <InputText id="NoRujukan" className="w-full mb-3 p-inputtext-sm" value={_entity?.NoRujukan} onChange={(e) => setValByKey("NoRujukan", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["NoRujukan"]) ? (
              <p className="m-0" key="error-NoRujukan">
                {error["NoRujukan"]}
              </p>
            ) : null}
          </small>
