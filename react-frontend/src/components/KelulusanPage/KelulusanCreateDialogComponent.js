...
const KelulusanCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.latihan)) {
                error["latihan"] = `Latihan field is required`;
                ret = false;
            }
    ...
    return (
        <Dialog header="Create Kelulusan" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                ...
                <InputText id="latihan" className="w-full mb-3 p-inputtext-sm" value={_entity?.latihan} onChange={(e) => setValByKey("latihan", e.target.value)}  required  />
            </span>
